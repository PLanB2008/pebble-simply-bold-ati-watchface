...
void size_weather() {
  ...
  if (weather_mode == 1) {
  ...
  } else { // ff Shaking is disabled --> This is what normally counts
    //layer_set_frame(text_layer_get_layer(temperature_layer), GRect(0, ((bounds.size.h / 2) - 10 - temperature_size.h), bounds.size.w, temperature_size.h));
    //                                                           Horizontal movement, vertical movement
    layer_set_frame(text_layer_get_layer(temperature_layer), GRect(56, (bounds.size.h / 2) + 24, bounds.size.w, temperature_size.h));
    ...
    // text_layer_set_text(conditions_layer, "Wolkenbedeckt");
    //                                                                              Minus 5 slightly moved to the left
    layer_set_frame(text_layer_get_layer(conditions_layer), GRect(PBL_IF_ROUND_ELSE(18-5, 0), PBL_IF_ROUND_ELSE(date_frame.origin.y + date_size.h, bounds.size.h - conditions_size.h - 5), 144, conditions_size.h));
  }
}
