...
GFont time_font, small_time_font, date_font, lg_weather_font, lg_condition_font, lg_temperature_font, sm_weather_font;
...
void size_time_layers() {
  ...
  layer_set_frame(text_layer_get_layer(date_layer), GRect(0, 2, bounds.size.w, date_size.h));
}
...
static void main_window_load(Window *window) {
  ...
  // Create fonts
  time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_SQUARE_55));
  small_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_SQUARE_30));
  ...
  lg_weather_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_BEBAS_18));
  lg_condition_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_BEBAS_18));
  lg_temperature_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_SQUARE_28));
  ...
  // background layer for conditions and date
  background_layer = text_layer_create(GRect(0, 0, bounds.size.h, bounds.size.h));
  text_layer_set_background_color(background_layer, GColorWhite);
  layer_set_frame(text_layer_get_layer(background_layer), GRect(0, (bounds.size.h / 2) + 27, bounds.size.w, 150));

  // zweiten parameter hochsetzen verschiebt bar nach unten
  middle_bar_layer = layer_create(GRect(0, 15, bounds.size.w, bounds.size.h));
  ...
  text_layer_set_font(temperature_layer, lg_temperature_font);
  //layer_set_frame(text_layer_get_layer(temperature_layer), GRect(0, 0, bounds.size.w, date_size.h));
  ...
  text_layer_set_font(conditions_layer, lg_condition_font);
