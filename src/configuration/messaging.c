...
static void inbox_recv_handler(DictionaryIterator *iter, void *ctx) {
  ...
  while(t) {
    ...
    if (true_key == CfgKeyTimeTextColour) {
      #ifdef DEBUG_MODE
      APP_LOG(APP_LOG_LEVEL_INFO, "Setting time_text_colour");
      #endif
      time_text_colour = t->value->int32;
    }

    if (true_key == CfgKeyDateTextColour) {
      #ifdef DEBUG_MODE
      APP_LOG(APP_LOG_LEVEL_INFO, "Setting date_text_colour");
      #endif
      date_text_colour = t->value->int32;
    }

    if (true_key == CfgKeyTemperatureTextColour) {
      #ifdef DEBUG_MODE
      APP_LOG(APP_LOG_LEVEL_INFO, "Setting temperature_text_colour");
      #endif
      temperature_text_colour = t->value->int32;
    }

    if (true_key == CfgKeyConditionTextColour) {
      #ifdef DEBUG_MODE
      APP_LOG(APP_LOG_LEVEL_INFO, "Setting condition_text_colour");
      #endif
      condition_text_colour = t->value->int32;
    }
